<!--
  ...
  Variant: design-md/sanity/DESIGN.md ("Design System Inspired by Sanity")
  Imported: 2026-05-11
  Selected for Venera Studio because Sanity's system mirrors Venera's brief almost exactly:
    - Near-black canvas (#0b0b0b) parallels Venera's --bg #0E0E0E
    - Monumental compressed display type with extreme negative tracking matches Venera's large display headlines
    - IBM Plex Mono "technical eyebrows" map directly onto Venera's mono-caps meta strips (Geist Mono in our stack)
    - A single warm accent (coral-red #f36458) reserved for the highest-priority CTA mirrors Venera's flame orange #FF4500
    - Editorial dark-first marketing surface with full-bleed sections + measured max-width content fits Venera's
      brutalist-ish, smilingwolf.co.uk-style portfolio chrome.
  Venera-specific overrides (apply when generating UI for this project):
    - Background:        #0E0E0E (Venera) instead of #0b0b0b (Sanity)
    - Primary foreground:#F5F0E8 (Venera warm off-white) instead of pure #ffffff
    - Primary CTA/accent:#FF4500 (Venera flame orange) instead of #f36458 (Sanity coral) and instead of #0052ef hovers
    - Display sans:      Geist     (already loaded via next/font) -- substitute for waldenburgNormal / Inter
    - Mono / meta:       Geist Mono (already loaded via next/font) -- substitute for IBM Plex Mono
  Everything else (spacing scale, radius scale, typographic tightness, depth-via-color, pill CTAs, mono uppercase
  technical labels, no drop shadows, generous 64-120px section breaks) should be followed as written below.
-->
